...
# save_crop_info 와 기능은 유사하고, 메모리 사용량을 줄인 것
# save_crop_info 차이점
#  1. min_idx, max_idx 를 df_anchor_i 에서 얻다가 df_fan 에서 얻도록 변경함
#     df_anchor_i 는 매 프레임 읽지 않는 경우가 있어서
#  2. 메모리 사용량 줄임 : imageio_ffmpeg 라이브러리를 사용해 stream 으로 읽음
#     이전보다 이미지가 좀 더 선명한 것 같음
...
# save_debug_clip 와 기능은 똑같고, 메모리 사용량을 줄임
# stream 으로 비디오를 만듦
def save_debug_clip2(clip, fps, verbose=False):
    jpgs = glob(f'{clip}/*.jpg')
    jpgs = sorted([(int(Path(e).stem.split('_')[0]), e) for e in jpgs])
    
    fan_pts = pd.read_pickle(Path(clip)/'df_fan.pickle')
    fan_pts = fan_pts.set_index('frame_idx')['cropped_pts2d']
    
    def draw_pts(im, pts, radius=1):
        im = im.copy()
        if pts is not None:
            for (x, y) in pts:
                cv2.circle(im, (x,y), radius=radius, color=(255, 0, 0), thickness=-1)
        return im
     
    writer = None
    save_path = f'{clip}/debug.mp4'
    for idx, f in tqdm(jpgs, desc='make debug.mp4', disable=not verbose):
        im = imageio.imread(f)
        radius = max(1, int(im.shape[0]/200))
        if idx == 0:
            print('radius:', radius)
        marked = draw_pts(im, fan_pts[idx], radius=radius) 
        merged = np.concatenate([im, marked], axis=1)

        sz = merged.shape[0]
        pw = (sz+1)//2*2 - sz
        merged = np.pad(merged, ((0,pw), (0,0), (0,0)), mode='constant', constant_values=128)
        #merged = [ for im in merged]
