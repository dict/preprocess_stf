...
def face_info_to_anchor(df, stride):
...
def save_face_info2(mp4_path, ebd_아나운서, base='./', verbose=False):
    ...
    if not Path(dst).exists():      
        ...
        #pdb.set_trace()
        df_face_info = face_info_to_anchor(df_)
...
# 메모리 적게 사용하도록 개선한 버전
def save_face_info3(mp4_path, ebd_아나운서, stride=None, base='./', callback=None, verbose=False):
    df_face_info_path = os.path.join(base,'df_face_info', f"{str(Path(mp4_path).stem)}.pickle")
    if verbose:
    ...
    if not Path(dst).exists():      
        ...
        #display(df_.groupby('frame_idx').count())
        #pdb.set_trace()
        df_face_info = face_info_to_anchor(df_, stride)
        df_face_info.to_pickle(dst)
        return [dst]
    callback2(100)
    return [dst]
